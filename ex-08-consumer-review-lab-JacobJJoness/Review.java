...
public class Review {
  ...
  public static double totalSentiment(String fileName) {
    ...
    double totalAmt = 0;
    String[] words = origReview.split("\\s+");
    for (int i = 0; i < words.length; i++) {
      String noPunc = removePunctuation(words[i]);
      totalAmt += sentimentVal(noPunc);
    }
    return totalAmt;
  }
  ...
  public static String fakeReview(String fileName) {
    ...
    while (review.indexOf("*") != -1) {
      ...
      int nextSpace = review.indexOf(" ", starIndex);
      String adj = review.substring(starIndex + 1, nextSpace);
      double adjVal = sentimentVal(adj);
      String randomAdj = randomAdjective();
      double randomVal = sentimentVal(randomAdj);
      if (totalSentiment(fileName) >= 0) {
        while (adjVal > randomVal) {
      ...
      review = review.substring(0, starIndex) + adj + review.substring(nextSpace);
    }
